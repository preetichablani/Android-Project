📱 Android Assignments Collection 🚀

 👩‍💻 Student Details

- Name: Preeti Chablani
- Course: Bachelor of Computer Applications
- Subject: Android Development

----

📌 Project Overview

This repository contains multiple Android assignments developed using Java and XML in Android Studio.  
These projects demonstrate various Android development concepts such as:
- UI Design
- User Input Handling
- ListView & GridView
- WebView Integration
- SharedPreferences
- Dialog Boxes
- SMS & Calling Features
- Splash Screen
- Calculator Logic


----

🛠️ Tech Stack

- ☕ Java
- 📱 Android Studio
- 🎨 XML (UI Design)

----

▶️ How to Run ⚙️

1. Open project in Android Studio
2. Build the project
3. Run on Emulator or Device
   
----


📂 Assignments Included 📚


 🔹 Assignment 1: User Details Form 📝

  Features
- Input fields for Name, City, Email, Mobile Number
- Data entry using EditText
- Simple Form UI

<img width="330" height="691" alt="Ass1" src="https://github.com/user-attachments/assets/0ba0e1e0-311d-4390-990e-5d2c2e4ec0cc" />

----

 🔹 Assignment 2: Name Input App 👤

  Features
- Enter and display user name
- Use of TextView and EditText

<img width="385" height="696" alt="Ass2" src="https://github.com/user-attachments/assets/8ba90c72-6f89-42e1-a794-d07022623409" />

----


 🔹 Assignment 3: Calculator App 🧮

  Features
- Addition
- Subtraction
- Multiplication
- Division
- Clean Calculator UI

<img width="347" height="730" alt="Ass3" src="https://github.com/user-attachments/assets/b9c27e4e-f5a0-4d9f-87b4-84c98f3d0080" />

----


 🔹 Assignment 4: Radio Button & Checkbox App ✅

 Features
- RadioButton for single selection
- CheckBox for multiple selection
- User choice handling

<img width="353" height="737" alt="Ass4" src="https://github.com/user-attachments/assets/c77cdb97-942d-4ee1-b1c4-2a18d1e96404" />

----


 🔹 Assignment 5: Alarm Clock App ⏰

 Features
- Set alarm time
- Basic alarm functionality
- Simple UI for time selection

<img width="365" height="807" alt="Ass5" src="https://github.com/user-attachments/assets/0f0e38de-462e-48c1-96f7-66b7ce2a5ad8" />


----
🔹 Assignment 6: To-Do List Application 📋

  Features
- Add Tasks using Dialog Box
- Delete Tasks using Long Press
- Tasks displayed using ListView

<img width="437" height="826" alt="image" src="https://github.com/user-attachments/assets/c1d4ddff-4032-479c-bed7-432dcd460401" />

----

🔹 Assignment 7: Image Picker App 🖼️

   Features
- Select Single/Multiple Images
- Display Images in GridView
- Custom Adapter Implementation


<img width="562" height="981" alt="image" src="https://github.com/user-attachments/assets/bb0e1a87-2bf2-4d73-a864-ddc46520c45c" />


----
 🔹 Assignment 8: Emergency Alert System 🚨

  Features
- Save Emergency Contacts
- Send Emergency SMS
- Direct Phone Call Feature

<img width="516" height="970" alt="image" src="https://github.com/user-attachments/assets/62144a32-7ef6-4a5b-8253-66b1b375a1f7" />

----


 🔹 Assignment 9: WebView Browser App 🌐
  Features
- Load URL inside App
- WebView Navigation
- Back Navigation Support

<img width="581" height="1114" alt="image" src="https://github.com/user-attachments/assets/684a804b-4bac-4518-90f5-a6e4e33dad09" />

----


🔹 Assignment 10: Calculator App with Splash Screen 🧮✨

  Features
- Number Buttons (0–9, 00)
- Arithmetic Operations (+, -, *, /, %)
- Decimal Support
- AC & Clear Functionality
- Splash Screen
- Responsive Calculator UI

<img width="550" height="1108" alt="image" src="https://github.com/user-attachments/assets/552d2f4b-81b1-4f71-abc2-c7be3d5dc88e" />

----


🎯 Learning Outcomes

✔️ Android UI components understanding

✔️ Handling user inputs

✔️ Working with RadioButton & CheckBox

✔️ Building logic (Calculator & Alarm)


----
 🚀 Key Features

✨ Beginner-Friendly Android Projects  
✨ Simple & Clean UI  
✨ Covers Important Android Concepts  
✨ Easy to Understand Code  
✨ Real Android App Implementations  

----
👩‍💻 Author

Preeti Chablani
----

⭐ Final Note

This repository is created for educational purposes and demonstrates basic Android development concepts.
